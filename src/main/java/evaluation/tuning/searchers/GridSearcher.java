...
public class GridSearcher extends ParameterSearcher {
    ...
    public class GridSearchIterator implements Iterator<ParameterSet> {
        ...
        public GridSearchIterator() {
            ...
            int i = 0;
            for (Map.Entry<String, List<String>> entry : space.getParameterLists().entrySet()) {
                keys[i] = entry.getKey();
        ...
        public ParameterSet next() {
            ...
            for (int i = 0; i < keys.length; i++)
                pset.parameterSet.put(keys[i], space.getParameterLists().get(keys[i]).get(currentInds[i]));
