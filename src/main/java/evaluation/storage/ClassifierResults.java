...
//REMAINDER OF THE FILE - 1 prediction per line
    ...
    */
    ...
    private void parseString(String str) throws Exception {
        ...
        parseFirstLine(lines.remove(0));
        parseSecondLine(lines.remove(0));
        double reportedTestAcc = parseThirdLine(lines.remove(0));

        //fileType was read in from first line.
        switch (fileType) {
            case PREDICTIONS: {
                //have all meta info, start reading predictions or metrics
                instancePredictionsFromStrings(lines);
            ...
            case METRICS:
                allPerformanceMetricsFromStrings(lines);
                break;
            case COMPACT:
    ...
    private void allPerformanceMetricsFromStrings(List<String> lines) {
        try {
            numClasses =    Integer.parseInt(lines.remove(0).split(",")[1]);
            numInstances =  Integer.parseInt(lines.remove(0).split(",")[1]);
            acc =           Double.parseDouble(lines.remove(0).split(",")[1]);
            balancedAcc =   Double.parseDouble(lines.remove(0).split(",")[1]);
            sensitivity =   Double.parseDouble(lines.remove(0).split(",")[1]);
            precision =     Double.parseDouble(lines.remove(0).split(",")[1]);
            recall =        Double.parseDouble(lines.remove(0).split(",")[1]);
            specificity =   Double.parseDouble(lines.remove(0).split(",")[1]);
            f1 =            Double.parseDouble(lines.remove(0).split(",")[1]);
            mcc =           Double.parseDouble(lines.remove(0).split(",")[1]);
            nll =           Double.parseDouble(lines.remove(0).split(",")[1]);
            meanAUROC =     Double.parseDouble(lines.remove(0).split(",")[1]);
            stddev =        Double.parseDouble(lines.remove(0).split(",")[1]);
            medianPredTime= Long.parseLong(lines.remove(0).split(",")[1]);

            assert(lines.remove(0) == "countPerClass");//todo change to if not throws
            countPerClass = new double[numClasses];
            for (int i = 0; i < numClasses; i++)
                countPerClass[i] = Double.parseDouble(lines.remove(0).split(",")[1]);

            assert(lines.remove(0) == "confusionMatrix"); //todo change to if not throws
            confusionMatrix = new double[numClasses][numClasses];
            for (int i = 0; i < numClasses; i++) {
                String[] vals = lines.remove(0).split(",");
    ...
    private void instancePredictionsFromStrings(List<String> lines) throws Exception {
        double correct = 0;
        while (!lines.isEmpty()) {
...
//            a/=numClasses;
    ...
    public static ClassifierResults parse(String string) throws Exception {
        ClassifierResults classifierResults = new ClassifierResults();
        classifierResults.parseString(string);
