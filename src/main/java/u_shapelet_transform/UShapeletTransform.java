...
public class UShapeletTransform extends SimpleBatchFilter implements SaveParameterInfo, Serializable {
	...
	protected boolean supressOutput; // defaults to print in System.out AS WELL as file, set to true to stop printing
										// to console
	 ...
	 */
	public UShapeletTransform(int k, int minShapeletLength, int maxShapeletLength,
			ShapeletQualityChoice qualityChoice) {
		this.numShapelets = k;
		...
		UShapeletSearchOptions sOp = new UShapeletSearchOptions.Builder().setMin(minShapeletLength)
	 ...
	 *
	 * @return An ArrayList of Shapelets representing the shapelets found for this
	 *         Shapelet Transform.
	 */
	 ...
	 * up the transform processing time.
	 *
	 * @param percentage the percentage of data to be precocessed before pruning is
	...
	/**
	 * The main logic of the filter; when called for the first time, k shapelets are
	 * extracted from the input Instances 'data'. The input 'data' is transformed by
	 * the k shapelets, and the filtered data is returned as an output.
	 * <p>
	 * If called multiple times, shapelet extraction DOES NOT take place again; once
	 * k shapelets are established from the initial call to process(), the k
	 * shapelets are used to transform subsequent Instances.
	 * <p>
	 * Intended use:
	 * <p>
	 ...
	 * training data;
	 * <p>
	 * 3. Use the filter to transform the raw testing data into transformed testing
	 * data (e.g. filter never extracts shapelets from training data, therefore
	 * avoiding bias);
	 * <p>
	 * 4. Build a classifier using transformed training data, perform classification
	 * on transformed test data.
	 *
	 * @param data the input data to be transformed (and to find the shapelets if
	 *             this is the first run)
	 * @return the transformed representation of data, according to the distances
	 *         from each instance to each of the k shapelets
	 */
	public Instances process(Instances data, Instances dataError) throws IllegalArgumentException {
		...
		// build the transformed dataset with the shapelets we've found either on this
		// data, or the previous training data
		return buildTansformedDataset(data, dataError);
	}

	protected void trainShapelets(Instances data, Instances dataErr) {
		// we might round robin the data in here. So we need to override the input data
	...
	private Instances initDataSource(Instances data) {
		...
		// shapelets discovery has not yet been caried out, so this must be training
		// data
	...
	public ArrayList<UShapelet> findBestKShapeletsCache(Instances data, Instances dataErr) {
		...
		for (; casesSoFar < dataSize; casesSoFar++) {
			...
			seriesShapelets = searchFunction.SearchForShapeletsInSeries(data.get(casesSoFar), dataErr.get(casesSoFar),
					this::checkCandidate);
	 ...
	 *         fitness (by infoGain, seperationGap then shortest length)
	 */
	public ArrayList<UShapelet> findBestKShapeletsCache(int numShapelets, Instances data, Instances dataErr,
	 ...
	 */
	protected ArrayList<UShapelet> combine(int k, ArrayList<UShapelet> kBestSoFar,
			ArrayList<UShapelet> timeSeriesShapelets) {
	...
	protected UShapelet checkCandidate(Instance series, Instance seriesErr, int start, int length, int dimension) {
		...
		UShapelet shapelet = new UShapelet(subseqDistance.getCandidate(), dataSourceIDs[casesSoFar], start,
	 ...
	 */
	public double timingForSingleShapelet(Instances data, Instances dataErr, int minShapeletLength,
	...
	public String getParameters() {
		String str = "minShapeletLength," + searchFunction.getMin() + ",maxShapeletLength," + searchFunction.getMax()
				+ ",numShapelets," + numShapelets + ",roundrobin," + roundRobin + ",searchFunction,"
				+ this.searchFunction.getClass().getSimpleName() + ",qualityMeasure,"
				+ this.quality.getQualityMeasure().getClass().getSimpleName();
		return str;
	}
	 ...
	 * @throws Exception
	 */
	public long opCountForSingleShapelet(Instances data, Instances dataErr, int minShapeletLength,
			int maxShapeletLength) throws Exception {
		data = roundRobinData(data, null);
		subseqDistOpCount = 0;
		findBestKShapeletsCache(1, data, dataErr, minShapeletLength, maxShapeletLength);
	...
	public static void main(String[] args) {
		try {
			...
			ShapeletSearchOptions searchOptions = new ShapeletSearchOptions.Builder().setMin(3)
					.setMax(train.numAttributes() - 1).setSearchType(ShapeletSearch.SearchType.FULL).build();

			ShapeletTransformFactoryOptions options = new ShapeletTransformFactoryOptions.Builder()
					.setDistanceType(SubSeqDistance.DistanceType.IMP_ONLINE).setKShapelets(train.numInstances() * 10)
					.useBinaryClassValue().useClassBalancing().useCandidatePruning().useRoundRobin()
					.setSearchOptions(searchOptions).build();
