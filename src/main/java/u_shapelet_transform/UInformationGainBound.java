...
public class UInformationGainBound extends UShapeletQualityBound {
     ...
     * @param isExact
     * */
    protected UInformationGainBound(ClassCounts classDist, int percentage, boolean isExact){
        initParentFields(classDist, percentage);
        this.isExact = isExact;
        parentEntropy = InformationGain.entropy(parentClassDist);
    }
    protected UInformationGainBound(ClassCounts classDist, int percentage){
    ...
    private double computeIG(Map<Double, Boolean> perm){
        ...
        double bsfGain = -1;
        UDistance lastDist = new UDistance(-1, 0);
        UDistance thisDist;
        double thisClassVal;
        int oldCount;
